...
{
    public class SyBaseClientAdapter: KdtAdapter
    {
        #region 构造函数

        public SyBaseClientAdapter()
            : base("", CommandType.Text)
        {
        }

        public SyBaseClientAdapter(string _connectionStr)
            : base(_connectionStr, CommandType.Text)
        { }

        public SyBaseClientAdapter(string _connectionStr, CommandType _cmdtype)
        ...
        /// 回滚方法
        /// </summary>
        private AseTransaction _sybaseTran { get; set; }

        /// <summary>
        /// Sql打开连接执行器
        /// </summary>
        private AseConnection _sybaseCn { get; set; }

        private delegate void AsyncExceReader(AseCommand cmd);
        ...
        public override bool IsOpened
        {
            get
            {
                if (_sybaseCn == null) return false;

                return !(_sybaseCn.State == ConnectionState.Broken || _sybaseCn.State == ConnectionState.Closed);
            }
        }
        ...
        public override void Open()
        {
            try
            {
                if (_sybaseCn != null && _sybaseCn.State == ConnectionState.Open)
                {
                    _sybaseCn.Close();
                }
                if (_sybaseCn == null)
                {
                    ...
                    _sybaseCn = new AseConnection(ConnectionStr);
                    _sybaseCn.Open();
                    isClosed = false; // 加入这一句修正数据库不能重复打开和关闭的问题!
                }
                if (_sybaseCn.State == ConnectionState.Closed)
                {
                    _sybaseCn.Open();
                }
            }
        ...
        public override void Close()
        {
            if (!isClosed)
            {
                if (IsAsync)
                {
                    ...
                    {
                        var sybaseadapter = adapter as SyBaseClientAdapter;
                        while (sybaseadapter.IsAsync) { Thread.Sleep(1000); }

                        if (sybaseadapter._sybaseTran != null)
                            sybaseadapter._sybaseTran.Rollback();

                        if (sybaseadapter._sybaseCn != null)
                        {
                            sybaseadapter._sybaseCn.Close();
                            sybaseadapter._sybaseCn.Dispose();
                        }

                        GC.SuppressFinalize(sybaseadapter);
                    });
        ...
        public override T ExecuteScalar<T>(string sql, KdtParameterCollection parameters)
        {
            try
            {
                ...
                // 执行SQL命令
                using (AseCommand cmd = new AseCommand(ReplaceSqlText(ReplaceSqlText(sql, parameters), parameters), _sybaseCn))
                {
                    InitCommand(cmd); // 初始化
        ...
        public override void BeginTrans()
        {
            ...
            try
            {
                _sybaseTran = _sybaseCn.BeginTransaction();
            }
            catch (Exception e)
            {
                throw new DataException(e.Message);
        ...
        /// <returns></returns>
        protected T GetEntity<T>(AseDataReader reader) where T : class, new()
        {
        ...
        protected List<KeyValueCollection> GetEntityCollection(AseDataReader reader)
        {
            List<KeyValueCollection> entitys = new List<KeyValueCollection>();
            // 读取列
            List<string> columns = new List<string>();
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        protected List<T> GetEntityPage<T>(AseDataReader reader, int offset, int limit) where T : class, new()
        {
            PropertyInfo[] properties = typeof(T).GetProperties();
            if (properties == null || properties.Length <= 0)
                throw new ArgumentNullException("实体类{0}不包含任何属性信息！".ToFormat(typeof(T).Name));
        ...
        /// 初始CMD执行器
        /// </summary>
        /// <param name="cmd">AseCommand执行器</param>
        private void InitCommand(AseCommand cmd)
        {
            if (_sybaseTran != null)
                cmd.Transaction = _sybaseTran;
        ...
        private Dictionary<string, AseParameter> ConvertToSqlParameter(KdtParameterCollection parameters)
        {
            Dictionary<string, AseParameter> result = new Dictionary<string, AseParameter>(StringComparer.OrdinalIgnoreCase);

            if (parameters != null && parameters.Count > 0)
            {
                foreach (var item in parameters)
                {
                    AseParameter param = new AseParameter("{0}{1}".ToFormat(ParamPrifix, item.Name), item.Value);
        ...
        /// </summary>
        /// <param name="parameters">参数集合</param>
        /// <param name="sybaseparams">转换后的参数集合</param>
        private void ReflectParamValue(KdtParameterCollection parameters, Dictionary<string, AseParameter> sybaseparams)
        {
            if (parameters != null && parameters.Count > 0)
            {
                foreach (var item in parameters)
                {
                    ...
                    {
                        item.Value = sybaseparams[item.Name].Value;
                    }
                }
            }
        }
