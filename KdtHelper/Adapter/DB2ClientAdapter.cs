...
{
    public  class DB2ClientAdapter: KdtAdapter
    {
        #region 构造函数

        public DB2ClientAdapter()
            : base("", CommandType.Text)
        {
        }

        public DB2ClientAdapter(string _connectionStr)
            : base(_connectionStr, CommandType.Text)
        { }

        public DB2ClientAdapter(string _connectionStr, CommandType _cmdtype)
        ...
        /// 回滚方法
        /// </summary>
        private DB2Transaction _db2Tran { get; set; }

        /// <summary>
        /// Sql打开连接执行器
        /// </summary>
        private DB2Connection _db2Cn { get; set; }
        ...
        public override bool IsOpened
        {
            get
            {
                if (_db2Cn == null) return false;

                return !(_db2Cn.State == ConnectionState.Broken || _db2Cn.State == ConnectionState.Closed);
            }
        }
        ...
        public override void Open()
        {
            try
            {
                ...
                _db2Cn = new DB2Connection(ConnectionStr);
        ...
        public override void Close()
        {
            if (!isClosed)
            {
                if (IsAsync)
                {
                    ...
                    {
                        var sqladapter = adapter as DB2ClientAdapter;
                        while (sqladapter.IsAsync) { Thread.Sleep(1000); }

                        if (sqladapter._db2Tran != null)
                            sqladapter._db2Tran.Rollback();

                        if (sqladapter._db2Cn != null)
                        {
                            sqladapter._db2Cn.Close();
                            sqladapter._db2Cn.Dispose();
                        }
        ...
        public override void BeginTrans()
        {
            ...
            try
            {
                _db2Tran = _db2Cn.BeginTransaction();
            }
            catch (Exception e)
            {
                throw new DataException(e.Message);
        ...
        /// <returns></returns>
        protected T GetEntity<T>(DB2DataReader reader) where T : class, new()
        {
        ...
        protected List<KeyValueCollection> GetEntityCollection(DB2DataReader reader)
        {
            List<KeyValueCollection> entitys = new List<KeyValueCollection>();
            // 读取列
            List<string> columns = new List<string>();
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        protected List<T> GetEntityPage<T>(DB2DataReader reader, int offset, int limit) where T : class, new()
        {
            PropertyInfo[] properties = typeof(T).GetProperties();
            if (properties == null || properties.Length <= 0)
                throw new ArgumentNullException("实体类{0}不包含任何属性信息！".ToFormat(typeof(T).Name));
        ...
        /// 初始CMD执行器
        /// </summary>
        /// <param name="cmd">DB2Command执行器</param>
        private void InitCommand(DB2Command cmd)
        {
            if (_db2Tran != null)
                cmd.Transaction = _db2Tran;
        ...
        private void ReflectParamValue(KdtParameterCollection parameters, Dictionary<string, DB2Parameter> sqlparams)
        {
            if (parameters != null && parameters.Count > 0)
            {
                foreach (var item in parameters)
