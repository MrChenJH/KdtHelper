...
{
    internal class OdbcClientAdapter : KdtAdapter
    {
        #region 构造函数

        public OdbcClientAdapter()
            : base("", CommandType.Text)
        {
        }

        public OdbcClientAdapter(string _connectionStr)
            : base(_connectionStr, CommandType.Text)
        { }

        public OdbcClientAdapter(string _connectionStr, CommandType _cmdtype)
        ...
        /// 回滚方法
        /// </summary>
        private OdbcTransaction _odbcTran { get; set; }

        /// <summary>
        /// Sql打开连接执行器
        /// </summary>
        private OdbcConnection _odbcCn { get; set; }
        ...
        public override bool IsOpened
        {
            get
            {
                if (_odbcCn == null) return false;

                return !(_odbcCn.State == ConnectionState.Broken || _odbcCn.State == ConnectionState.Closed);
            }
        }
        ...
        public override void Open()
        {
            try
            {
                ...
                _odbcCn = new OdbcConnection(ConnectionStr);
        ...
        public override void Close()
        {
            if (!isClosed)
            {
                if (IsAsync)
                {
                    ...
                    {
                        var odbcadapter = adapter as OdbcClientAdapter;
                        while (odbcadapter.IsAsync) { Thread.Sleep(1000); }

                        if (odbcadapter._odbcTran != null)
                            odbcadapter._odbcTran.Rollback();

                        if (odbcadapter._odbcCn != null)
                        {
                            odbcadapter._odbcCn.Close();
                            odbcadapter._odbcCn.Dispose();
                        }

                        GC.SuppressFinalize(odbcadapter);
                    });
        ...
        public override void BeginTrans()
        {
            ...
            try
            {
                _odbcTran = _odbcCn.BeginTransaction();
            }
            catch (Exception e)
            {
                throw new DataException(e.Message);
        ...
        /// <returns></returns>
        protected T GetEntity<T>(OdbcDataReader reader) where T : class, new()
        {
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        protected KeyValueCollection GetEntity(OdbcDataReader reader)
        {
            KeyValueCollection entity = new KeyValueCollection();
            // 读取列
        ...
        protected List<KeyValueCollection> GetEntityCollection(OdbcDataReader reader)
        {
            List<KeyValueCollection> entitys = new List<KeyValueCollection>();
            // 读取列
            List<string> columns = new List<string>();
        ...
        /// <param name="reader"></param>
        /// <returns></returns>
        protected List<T> GetEntityPage<T>(OdbcDataReader reader, int offset, int limit) where T : class, new()
        {
            PropertyInfo[] properties = typeof(T).GetProperties();
            if (properties == null || properties.Length <= 0)
                throw new ArgumentNullException("实体类{0}不包含任何属性信息！".ToFormat(typeof(T).Name));
        ...
        /// 初始CMD执行器
        /// </summary>
        /// <param name="cmd">OdbcCommand执行器</param>
        private void InitCommand(OdbcCommand cmd)
        {
            if (_odbcTran != null)
                cmd.Transaction = _odbcTran;
        ...
        /// </summary>
        /// <param name="parameters">参数集合</param>
        /// <returns>返回转换成OdbcParameter集合</returns>
        private Dictionary<string, OdbcParameter> ConvertToOdbcParameter(KdtParameterCollection parameters)
        {
            Dictionary<string, OdbcParameter> result = new Dictionary<string, OdbcParameter>(StringComparer.OrdinalIgnoreCase);

            if (parameters != null && parameters.Count > 0)
            {
                foreach (var item in parameters)
                {
                    OdbcParameter param = new OdbcParameter("{0}{1}".ToFormat(ParamPrifix, item.Name), item.Value);
        ...
        private void ReflectParamValue(KdtParameterCollection parameters, Dictionary<string, OdbcParameter> sqlparams)
        {
            if (parameters != null && parameters.Count > 0)
            {
                foreach (var item in parameters)
